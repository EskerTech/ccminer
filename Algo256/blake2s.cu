...
extern "C" void blake2s_hash(void *output, const void *input) {
	...
	h[0] = 0x01010020 ^ blake2s_IV[0];	h[1] = blake2s_IV[1];
...
__constant__ uint32_t _ALIGN(16) midstate[20];
...
#pragma unroll
		...
		v[12] = ROL16(v[12] ^ v[1]);		v[13] = ROL16(v[13] ^ v[2]);		v[15] = ROL16(v[15] ^ v[0]);
	...
	do {
		...
		if (h_resNonce[thr_id][0] != 0) {
			...
			for (i = 1; i<h_resNonce[thr_id][0] + 1; i++) {
				uint32_t vhashcpu[8];
				uint32_t nonce = sph_bswap32(h_resNonce[thr_id][i]);
				be32enc(&endiandata[19], nonce);
				blake2s_hash(vhashcpu, endiandata);
				if (vhashcpu[6] <= ptarget[6] && fulltest(vhashcpu, ptarget)) {
					work_set_target_ratio(work, vhashcpu);
					*hashes_done = pdata[19] - first_nonce + throughput;
					pdata[19] = work->nonces[0] = nonce;
					rc = 1;
					//search for 2nd nonce
					for (uint32_t j = i + 1; j<h_resNonce[thr_id][0] + 1; j++) {
						nonce = sph_bswap32(h_resNonce[thr_id][j]);
						be32enc(&endiandata[19], nonce);
						blake2s_hash(vhashcpu, endiandata);
						if (vhashcpu[6] <= ptarget[6] && fulltest(vhashcpu, ptarget)) {
							work->nonces[1] = nonce;
							//							if(!opt_quiet)
							//								gpulog(LOG_BLUE,dev_id,"Found 2nd nonce: %u/%08X - %u/%08X",i,pdata[19],j,work->nonces[1]);
