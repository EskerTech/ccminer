...
static _ALIGN(16) uint32_t *d_hash[MAX_GPUS];
static _ALIGN(16) uint32_t *d_resNonce[MAX_GPUS];
static _ALIGN(16) uint32_t *h_resNonce[MAX_GPUS];
...
extern "C" int scanhash_x17(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	CUDA_SAFE_CALL(cudaMemset(d_resNonce[thr_id], 0xff, NBN*sizeof(uint32_t)));
