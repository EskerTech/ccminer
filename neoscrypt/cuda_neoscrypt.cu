...
__global__ __launch_bounds__(TPB2, 1) void neoscrypt_gpu_hash_start(uint32_t threads, uint32_t startNonce)
{
...
#error TPB2 too low
	...
	const uint32_t ZNonce = cuda_swab32(nonce); //freaking morons !!!
...
__global__ __launch_bounds__(TPB2, 8) void neoscrypt_gpu_hash_ending( uint32_t threads, uint32_t startNonce, uint32_t *nonceVector)
{
	__shared__ uint32_t s_data[64];
...
#error TPB2 too low
	...
	const uint32_t ZNonce = cuda_swab32(nonce);
...
__host__ void neoscrypt_cpu_hash_k4_2stream(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *result)
{
	...
	neoscrypt_gpu_hash_ending << <grid2, block2 >> >(threads, startNounce, d_NNonce[thr_id]); //fastkdf+end
