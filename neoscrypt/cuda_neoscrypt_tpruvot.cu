...
void neoscrypt_gpu_hash_start(const uint32_t startNonce)
{
	__shared__ uint32_t s_data[64 * TPB2];

	const uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	const uint32_t ZNonce = cuda_swab32(startNonce + thread); 
...
void neoscrypt_gpu_hash_ending(const uint32_t startNonce, uint32_t *resNonces)
{
	__shared__ uint32_t s_data[64 * TPB2];
...
void neoscrypt_hash_tpruvot(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *resNonces)
{
	...
	neoscrypt_gpu_hash_start << <grid2, block2 >> > (startNounce); //fastkdf

	neoscrypt_gpu_hash_salsa1 << <grid3, block3 >> > ();
	neoscrypt_gpu_hash_chacha1 << <grid3, block3 >> > ();

	neoscrypt_gpu_hash_ending << <grid2, block2 >> > (startNounce, d_NNonce[thr_id]); //fastkdf+end
